...
export const Estimates = () => {
  ...
  const generatePDF = (estimate) => {
    ...
    let yPos = 58;
    // Prefer Company Name, fallback to User Name, then "Moja Firma"
    doc.setFont("helvetica", "bold");
    doc.text(user?.companyName || user?.name || "Moja Firma", 120, yPos);
    ...
    if (user?.phone) {
        doc.text(`Tel: ${user.phone}`, 120, yPos);
        yPos += 6;
    }

    doc.text(user?.email || "kontakt@firma.pl", 120, yPos);
    yPos += 6;

    if (user?.bankAccount) {
        yPos += 2;
        doc.setFontSize(10);
        doc.text(`Konto: ${user.bankAccount}`, 120, yPos);
    }
